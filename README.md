...
## 📊 Типичный сценарий без прокси

### **DNS-запросы при стандартной загрузке:**
```html
<link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;700&display=swap" rel="stylesheet">
```

**Последовательность DNS-запросов:**
1. **fonts.googleapis.com** → получение CSS (~40–60 ms)
2. **fonts.gstatic.com** → загрузка файлов шрифтов (~40–60 ms)
3. Возможные дополнительные поддомены для разных форматов

### **Время DNS-резолвинга по регионам:**
* **Локальные ISP**: 40–60 ms
* **Международные запросы**: 60–100 ms
* **Мобильные сети**: 50–100 ms
* **Медленные соединения**: 120–200 ms+

## ⚡ Экономия с прокси

### **DNS-запросы с прокси:**
```html
<link href="/fonts-proxy.php?family=Inter:wght@400;700&display=swap" rel="stylesheet">
```

**Результат:** ❌ **НОЛЬ дополнительных DNS-запросов**
...
```
Без прокси:
* DNS fonts.googleapis.com: ~40–60 ms
* DNS fonts.gstatic.com: ~40–60 ms
* Итого: ~80–120 ms + задержка сети

С прокси:
- DNS: 0ms (используется текущий домен)
- Экономия: 80–120 ms+
```

### **Для множественных шрифтов:**
```css
/* Типичная ситуация на сайте */
...
**Без прокси:** 6-9 DNS-запросов (googleapis.com + gstatic.com для каждого)
**С прокси:** 0 дополнительных DNS-запросов
...
* **США/Европа**: 40–60 ms экономии
* **Азия**: 60–100 ms экономии
* **Африка/Латинская Америка**: 80–150 ms экономии
* **Удалённые регионы**: 120–200 ms+ экономии

## 📱 Особенности мобильных сетей

### **Мобильный DNS-резолвинг:**
* **3G**: 200–400 ms на DNS‑запрос
* **4G**: 50–100 ms на DNS‑запрос
* **5G**: 20–50 ms на DNS‑запрос

**Экономия для мобильных пользователей может достигать 200–400 ms!**

## 🔄 Дополнительные факторы экономии

### **1. Исключение HTTP-редиректов:**
```
fonts.googleapis.com → fonts.gstatic.com (редирект)
+ дополнительный DNS для gstatic.com
```

### **2. Предотвращение повторных DNS-запросов:**

Кэширование предотвращает повторные обращения к Google

### **3. Пользовательский кэш DNS:**
- DNS TTL Google Fonts: ~300 секунд
- С локальным прокси: DNS TTL вашего домена (обычно выше)

## 📊 Измеримые метрики улучшения

### **Waterfall-анализ:**

**До (с внешними шрифтами):**
```
1. DNS lookup fonts.googleapis.com     [████████] 50 ms
2. Connect fonts.googleapis.com        [██] 20 ms  
3. Request CSS                         [███] 80 ms
4. DNS lookup fonts.gstatic.com        [████████] 50 ms
5. Connect fonts.gstatic.com           [██] 20 ms
6. Download fonts                      [█████] 120 ms
Итого: ~340 ms
```

**После (с прокси):**
```
1. Request CSS (same domain)           [███] 80 ms
2. Download fonts (same domain)        [█████] 120 ms  
Итого: ~200 ms
...
* **Start Render**: улучшение на 50–150 ms
* **First Contentful Paint**: улучшение на 40–120 ms
* **Largest Contentful Paint**: улучшение на 50–150 ms

### **Lighthouse метрики:**
* **Eliminate render-blocking resources**: +1–5 points
* **Reduce unused CSS**: автоматическая оптимизация
* **Serve static assets with efficient cache policy**: +1–10 points

## 💰 Экономический эффект

### **Для высоконагруженных сайтов:**
* 1000 посетителей/день × 100 ms экономии = 100 s (≈1.7 мин) сэкономленного времени ежедневно
* Улучшение конверсии на 0.5–1 % за счёт ускорения загрузки
* Снижение показателя отказов на 3–5 %

### **SEO преимущества:**
- Улучшение Core Web Vitals
- Повышение позиций в поиске Google
- Лучший пользовательский опыт

**Вывод:** Экономия на DNS-резолвинге составляет 40–200 ms в зависимости от региона пользователя и количества используемых шрифтов, что критически важно для современной веб-производительности.
